        ...
        if (NULL != f) {
            ...
            while (!feof(f) && idx + 1 < _SURV_MAX_WINDOW_NUM) {
                ...
                if (0 != strcmp(szURL, "\0")) {
                    ...
                    pVideoWindow->pDlgVideo[idx].Play(szURL);
                    idx++;
                }
            }
        }
...
void	CEasyPlayerDlg::CreateComponents()
{
    ...
    if (NULL != pComboxSplitScreen) {
        ...
        pComboxSplitScreen->SetCurSel(1);
        OnCbnSelchangeComboSplitScreen();
    }
...
void	CEasyPlayerDlg::UpdateComponents()
{
    ...
    CRect	rcVideo;
    rcVideo.SetRect(rcClient.left + 2, rcClient.top + 36, rcClient.right - 2, rcClient.bottom - 60);
