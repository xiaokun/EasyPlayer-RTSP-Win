...
void CDlgVideo::OnBnClickedButtonPreview()
{
    if (m_ChannelId > 0) {
        ...
        wchar_t wszURL[128] = { 0, };
        if (NULL != pEdtURL)
            pEdtURL->GetWindowTextW(wszURL, sizeof(wszURL));
...
void CDlgVideo::Play(char* szURL)
{
    ...
    m_ChannelId = EasyPlayer_OpenStream(szURL, hWnd, (RENDER_FORMAT)RenderFormat, nRtpOverTcp,
        szUsername, szPassword, &CDlgVideo::EasyPlayerCallBack, this, bHardDecode, NULL, NULL);
    if (m_ChannelId > 0) {
        ...
        if (NULL != pBtnPreview)
            pBtnPreview->SetWindowText(TEXT("Stop"));
    }
}
